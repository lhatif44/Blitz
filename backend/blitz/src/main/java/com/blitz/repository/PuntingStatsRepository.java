...
public interface PuntingStatsRepository extends JpaRepository<PuntingStats, UUID> {
    ...
    //Function to get every punting stat row for a season/season type in one query — used by ingestion to
    //prefetch existing rows instead of querying once per CSV row
    List<PuntingStats> findBySeasonAndSeasonType(Integer season, String seasonType);
}
